<header class="sticky top-0 z-999 flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
    <div class="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
        <div class="flex items-center gap-2 sm:gap-4 2xl:hidden">
            <!-- Hamburger Toggle BTN -->
            <button
                class="z-99999 block rounded-sm border border-stroke bg-white p-1.5 shadow-sm dark:border-strokedark dark:bg-boxdark 2xl:hidden"
            ...
            <!-- Hamburger Toggle BTN -->
            <a class="block flex-shrink-0 2xl:hidden" href="{{ route('home.index') }}">
                <img src="{{ asset('images/logo/logo.png') }}" class="h-9" alt="Logo" />
            </a>
        </div>
            ...
            <form action="https://formbold.com/s/unique_form_id" method="POST">
                <div class="relative">
                    ...
                    <input type="text" placeholder="Escribe para buscar ..."
                        class="w-full bg-transparent border-0 pl-9 pr-4 focus:outline-none focus:ring-0 focus:border-0 xl:w-125" />
                </div>
            </form>
        </div>
        ...
        <div class="flex items-center gap-3 2xsm:gap-7">
            <ul class="flex items-center gap-2 2xsm:gap-4">
                ...
                <li class="relative" x-data="{ dropdownOpen: false, notifying: {{ count($eventosHoy) > 0 ? 'true' : 'false' }} }" @click.outside="dropdownOpen = false">
                    ...
                    <!-- Dropdown Start - Diseño mejorado -->
                    <div x-show="dropdownOpen"
                        class="absolute -right-27 mt-2.5 flex h-auto w-75 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark sm:right-0 sm:w-80">
                        <div class="px-4.5 py-3">
                            <h5 class="text-sm font-medium text-bodydark2">
                                <div class="flex justify-between items-center">
                                    <div class="flex items-center gap-1.5">
                                        <span class="text-sm font-medium">Eventos de Hoy</span>
                                    </div>
                                    <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-brand-500 text-xs font-medium text-white">{{ count($eventosHoy) }}</span>
                                </div>
                                <span class="block text-xs text-gray-500 mt-1 dark:text-gray-400">
                                    {{ \Carbon\Carbon::now()->locale('es')->isoFormat('dddd, D [de] MMMM [de] YYYY') }}
                                </span>
                            </h5>
                        </div>

                        <div class="max-h-70 overflow-y-auto custom-scrollbar">
                            @forelse($eventosHoy as $evento)
                            <div class="border-t border-stroke dark:border-strokedark hover:bg-gray-50 dark:hover:bg-meta-4 transition duration-300">
                                <a href="{{ route('calendar.index') }}?event={{ $evento->id }}&openEvent=true" 
                                   class="block p-4">
                                    <div class="flex justify-between items-start mb-1">
                                        <h6 class="text-sm font-medium text-gray-900 dark:text-white">{{ $evento->titulo }}</h6>
                                        @php
                                            $statusClass = '';
                                            $statusText = '';
                                        ...
                                        @endphp
                                        <span class="px-2 py-0.5 text-xs font-medium rounded border {{ $statusClass }}">{{ $statusText }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 mb-1">
                                        <svg class="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd"></path>
                                        </svg>
                                        <span class="text-xs text-gray-600 dark:text-gray-300">{{ $evento->cliente_nombre ?: 'Sin cliente asignado' }}</span>
                                    </div>
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        <span class="text-xs text-gray-600 dark:text-gray-300">
                                            {{ $evento->todo_dia ? 'Todo el día' : \Carbon\Carbon::parse($evento->fecha_inicio)->format('H:i') }}
                                        </span>
                                    </div>
                                    @if($evento->descripcion)
                                    <p class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">{{ $evento->descripcion }}</p>
                                    @endif
                                </a>
                            </div>
                            @empty
                            <div class="flex flex-col h-32 justify-center items-center py-8">
                                <svg class="w-12 h-12 mb-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                </svg>
                                <p class="text-sm text-gray-500 dark:text-gray-400">No hay eventos programados para hoy</p>
                            </div>
                            @endforelse
                        </div>

                        <div class="px-4.5 py-3 border-t border-stroke dark:border-strokedark">
                            <a href="{{ route('calendar.index') }}" class="flex items-center justify-center gap-1 text-sm font-medium text-brand-500 hover:text-brand-600 dark:text-white/70 dark:hover:text-white">
                                <span>Ver calendario completo</span>
                                <svg class="fill-current" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                    <path d="M12.172 7L6.808 1.636L8.222 0.222L16 8L8.222 15.778L6.808 14.364L12.172 9H0V7H12.172Z" fill=""></path>
                                </svg>
                            </a>
                        </div>
                    </div>
                ...
                <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = !dropdownOpen">
                    @auth
                        ...
                        <span class="hidden text-right lg:block">
                            ...
                            <span class="block text-xs font-medium">{{ $rol->nombre_rol }}</span>
                        </span>
                    @endauth
                        ...
                        <button type="submit"
                            ...
                            <svg class="fill-current" width="22" height="22" viewBox="0 0 22 22"
                                ...
                                <path
                                    ...
                                    fill="" />
                                <path
                                    d="M6.05001 11.7563H12.2031C12.6156 11.7563 12.9594 11.4125 12.9594 11C12.9594 10.5875 12.6156 10.2438 12.2031 10.2438H6.08439L8.21564 8.07813C8.52501 7.76875 8.52501 7.2875 8.21564 6.97812C7.90626 6.66875 7.42501 6.66875 7.11564 6.97812L3.67814 10.4844C3.36876 10.7938 3.36876 11.275 3.67814 11.5844L7.11564 15.0906C7.25314 15.2281 7.45939 15.3312 7.66564 15.3312C7.87189 15.3312 8.04375 15.2625 8.21564 15.125C8.52501 14.8156 8.52501 14.3344 8.21564 14.025L6.05001 11.7563Z"
                                    fill="" />
                            </svg>
                            Cerrar Sesión
                        </button>
